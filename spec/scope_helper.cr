...
module PlaceOS::Api
  macro test_controller_scope(klass)
    ...
    context "read" do
      it "allows access to show" do
        ...
        result = show_route({{ base }}, id, diff_authorization_header)
      ...
      it "allows access to index" do
        ...
        result = index_route({{ base }}, diff_authorization_header)
      ...
      it "should not allow access to create" do
        ...
        result = create_route({{ base }}, body, diff_authorization_header)
      ...
      it "should not allow access to delete" do
        ...
        model.persisted?.should be_true
        id = model.id.as(String)
        result = delete_route({{ base }}, id, diff_authorization_header)
        result.status_code.should eq 403
  ...
  macro test_update_write_scope(klass)
    ...
    _, diff_authorization_header = authentication(scope: [PlaceOS::Model::UserJWT::Scope.new({{scope_name}}, PlaceOS::Model::UserJWT::Scope::Access::Write)])
          ...
          id = model.id.as(String)
          path = base + id
          result = update_route(path, model, diff_authorization_header)
          ...
          _, diff_authorization_header = authentication(scope: [PlaceOS::Model::UserJWT::Scope.new({{scope_name}}, PlaceOS::Model::UserJWT::Scope::Access::Read)])
          result = update_route(path, model, diff_authorization_header)
...
def show_route(base, id, diff_authorization_header)
  curl(
    method: "GET",
    path: base + id,
...
def index_route(base, diff_authorization_header)
  curl(
    method: "GET",
    path: base,
...
def create_route(base, body, diff_authorization_header)
  curl(
    method: "POST",
    path: base,
    body: body,
...
def delete_route(base, id, diff_authorization_header)
  curl(
    method: "DELETE",
    path: base + id,
    headers: diff_authorization_header,
...
def update_route(path, body, diff_authorization_header)
  curl(
    method: "PATCH",
    path: path,
    body: body.to_json,
